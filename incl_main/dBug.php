...
class d {
	...
	//constructor
	function d($var, $add_str = "", $forceType="") {
		...
		if(in_array($forceType,$arrAccept))
			...
			$this->checkType($var,$name_of_var);
	}
	...
	function makeTableHeader($type,$header,$colspan=2) {
		echo "<table cellspacing=2 cellpadding=3 class=\"dBug_".$type."\">
				<tr>
					<td class=\"dBug_".$type."Header\" colspan=".$colspan." style=\"cursor:hand\" onClick='dBug_toggleTable(this)'>".$header . ( $this->add_str ? '   ' . $this->add_str : '' )."</td>
				</tr>";
	}
	...
	//check variable type
	function checkType($var, $name_of_var="") {
		switch(gettype($var)) {
			case "resource":
			...
			default:
				...
				echo "<table cellspacing=0 border=2><tr>\n<td>".$this->add_str."</td>\n<td>".$var ."</td>\n</tr>\n</table>\n";
				break;
		}
	}
